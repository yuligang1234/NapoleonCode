    ...
    public class BaseTemplate
    {
        ...
        public static string InsertBaseModel(AppConfig appConfig)
        {
            ...
            if (dataBaseTables.Rows.Count > 0)
            {
                foreach (DataRow row in dataBaseTables.Rows)
                {
                    ...
                    sb.AppendFormat("private {0} {1}{2}", PublicFun.FormatType(0, row["DataType"].ToString()), privateStr, PublicFiled.WarpSymbol);
                    sb.AppendFormat("{0}", PublicFun.GetCommentary(row["TableDesc"].ToString()));//注释
                    sb.AppendFormat("public {0} {1}{2}", PublicFun.FormatType(0, row["DataType"].ToString()), row["TableColumn"], PublicFiled.WarpSymbol);
                    sb.Append("{");
                    sb.AppendFormat("{0}", PublicFiled.WarpSymbol);
        ...
        public static string InsertBaseProcedure(AppConfig appConfig)
        {
            ...
            if (dataBaseTables.Rows.Count > 0)
            {
                ...
                sb.Append("{");
                sb.AppendFormat("{0}", PublicFiled.WarpSymbol);
                sb.AppendFormat("{0}@SYS_ACTION nvarchar(8),  --\"ADD\"表示增加,\"UPD\"表示修改,\"DEL\"表示删除,\"INF\"表示获取基本信息,\"LST\"表示获取全部列表数据{1}{0}@OutId int = NULL out, --插入数据成功以后，返回插入数据对应的自增字段ID值{1}{0}@RunRowCount int = NULL out, --返回除SELECT操作外的脚本执行以后表受影响的行数{1}", PublicFiled.TabSymbol, PublicFiled.WarpSymbol);
                int i = 0;//计数
                foreach (DataRow row in dataBaseTables.Rows)
                {
                    i++;
                    ...
                    sb.AppendFormat("{0}@{1} {2}=NULL{3}{4}--{5}", PublicFiled.TabSymbol, row["TableColumn"], PublicFun.FormatType(row["DataType"].ToString(), row["TypeLength"].ToString(),
                        row["DecimalLength"].ToString()), i == dataBaseTables.Rows.Count ? "" : ",", PublicFiled.TabSymbol, row["TableDesc"]);
                    sb.AppendFormat("{0}", PublicFiled.WarpSymbol);
                }
                sb.Append("}");
                ...
                sb.AppendFormat("{0}{0}INSERT INTO {1}({2}) VALUES({3}){4}", PublicFiled.TabSymbol, PublicFiled.TableName, sb1.ToString().Trim(','), sb2.ToString().Trim(','), PublicFiled.WarpSymbol);
                ...
                sb.AppendFormat("{0}BEGIN{1}", PublicFiled.TabSymbol, PublicFiled.WarpSymbol);
                sb.AppendFormat("{0}{0}UPDATE {1} SET {2} WHERE {3}{4}", PublicFiled.TabSymbol, PublicFiled.TableName, sb3.ToString().Trim(','), sb4, PublicFiled.WarpSymbol);
                ...
                sb.AppendFormat("{0}BEGIN{1}", PublicFiled.TabSymbol, PublicFiled.WarpSymbol);
                sb.AppendFormat("{0}{0}DELETE {1} WHERE {2}{3}", PublicFiled.TabSymbol, PublicFiled.TableName, sb4, PublicFiled.WarpSymbol);
                sb.AppendFormat("{0}{0}SET @RunRowCount = @@ROWCOUNT{1}", PublicFiled.TabSymbol, PublicFiled.WarpSymbol);
                sb.AppendFormat("{0}END{1}", PublicFiled.TabSymbol, PublicFiled.WarpSymbol);
                //单条查询
                sb.AppendFormat("IF @SYS_ACTION = 'INF'{0}", PublicFiled.WarpSymbol);
                sb.AppendFormat("{0}BEGIN{1}", PublicFiled.TabSymbol, PublicFiled.WarpSymbol);
                sb.AppendFormat("{0}{0}SELECT {1} FORM {2} WHERE {3}{4}", PublicFiled.TabSymbol, sb1.ToString().Trim(','), PublicFiled.TableName, sb4, PublicFiled.WarpSymbol);
                sb.AppendFormat("{0}{0}SET @RunRowCount = @@ROWCOUNT{1}", PublicFiled.TabSymbol, PublicFiled.WarpSymbol);
                sb.AppendFormat("{0}END{1}", PublicFiled.TabSymbol, PublicFiled.WarpSymbol);
                //列表查询
                sb.AppendFormat("IF @SYS_ACTION = 'LST'{0}", PublicFiled.WarpSymbol);
                sb.AppendFormat("{0}BEGIN{1}", PublicFiled.TabSymbol, PublicFiled.WarpSymbol);
                sb.AppendFormat("{0}{0}SELECT {1} FORM {2} {3}", PublicFiled.TabSymbol, sb1.ToString().Trim(','), PublicFiled.TableName, PublicFiled.WarpSymbol);
                sb.AppendFormat("{0}{0}SET @RunRowCount = @@ROWCOUNT{1}", PublicFiled.TabSymbol, PublicFiled.WarpSymbol);
                sb.AppendFormat("{0}END{1}", PublicFiled.TabSymbol, PublicFiled.WarpSymbol);
                sb.AppendFormat("{0}", PublicFiled.WarpSymbol);
            }
