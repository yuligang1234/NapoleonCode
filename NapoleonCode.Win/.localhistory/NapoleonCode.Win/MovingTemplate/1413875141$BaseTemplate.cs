    ...
    public class BaseTemplate
    {
        ...
        public static string InsertSqLiteBaseModel(AppConfig appConfig, string tableName)
        {
            ...
            if (dataBaseTables.Rows.Count > 0)
            {
                ...
                {
                    //私有属性,_+首字母小写
                    string privateStr = string.Format("_{0}{1}", row["name"].ToString().ToLower().Substring(0, 1),
                        row["name"].ToString().Substring(1));
                    sb.AppendFormat("private {0} {1}{2}", PublicFun.FormatType(0, row["type"].ToString()),
                        privateStr, PublicFiled.WarpSymbol);
                    sb.AppendFormat("{0}", ""); //注释
                    sb.AppendFormat("public {0} {1}{2}", PublicFun.SqLiteFormatType(0, row["type"].ToString()),
                        row["name"], PublicFiled.WarpSymbol);
                    sb.Append("{");
                    sb.AppendFormat("{0}", PublicFiled.WarpSymbol);
                    sb.AppendFormat("{0}get {1} return {2};{3}{4}", PublicFiled.TabSymbol, "{", privateStr, "}",
                        PublicFiled.WarpSymbol);
