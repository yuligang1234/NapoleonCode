...
namespace NapoleonCode.Win
{
    public partial class DbSqLite : BaseForm
    {
        ...
        {
            //清除选中状态
            RadBaseModelXml.Checked = RadBaseField.Checked = RadBaseModel.Checked = RadBaseProcedure.Checked = RadNhMapping.Checked = RadNhModel.Checked = RadNhXml.Checked = false;
            if (CobSelectTemb.SelectedIndex == 0)//基础模版
            {
                PlBaseTemp.Visible = true;
                PlNhTemp.Visible = PlBaseTemp.Visible = false;
            }
            else if (CobSelectTemb.SelectedIndex == 1)//NHibernate模版
            {
                PlNhTemp.Visible = true;
                PlBaseTemp.Visible = PlBaseTemp.Visible = false;
                //加载对应的命名空间
